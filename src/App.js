...
import { Search, Phone, MapPin, Eye, Star, TrendingUp, CheckCircle, LogOut, Bot, MessageSquare, Link as LinkIcon, Building, Globe, Download, AlertTriangle, ChevronDown, BarChart2 } from 'lucide-react';
import { jsPDF } from 'jspdf';
import html2canvas from 'html2canvas';


// MOCK API - Replace with actual API calls to your backend
const mockApi = {
  getLocations: async () => {
    return [
  ...
  getReportData: async (locationId) => {
    // This function now returns different data based on the locationId
    const baseData = {
      '1': {
...
const SearchViewsChart = ({ data }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 h-full">
      <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><BarChart2 className="w-5 h-5 mr-2 text-gray-400"/>Search Views Breakdown</h3>
      <div className="h-64 w-full">
       ...
       <div className="flex justify-center mt-4 space-x-4 text-sm">
        <div className="flex items-center"><span className="w-3 h-3 rounded-full bg-sky-500 mr-2"></span>Direct: {data.direct}</div>
        <div className="flex items-center"><span className="w-3 h-3 rounded-full bg-indigo-500 mr-2"></span>Discovery: {data.discovery}</div>
      </div>
    </div>
  );
};
...
const App = () => {
  ...
  const handleDownloadPdf = () => {
    ...
    html2canvas(input, {
        ...
        useCORS: true, 
        logging: true,
        backgroundColor: '#f0f2f5' // Match body bg
    }).then(canvas => {
  ...
  const selectedLocationName = locations.find(l => l.id === selectedLocation)?.name || "Select a Location";
    ...
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-6 flex items-center space-x-3">
          <img src="https://gbprocket.com/wp-content/uploads/sites/2/2024/05/logo-blacktext-scaled.png" alt="GBP Rocket Logo" className="h-8 w-auto" />
        </div>
        ...
        <div className="p-4 border-t border-gray-200">
           <button className="flex items-center w-full px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-lg">
      ...
      <main className="flex-1 p-8">
        <header className="flex justify-between items-center mb-8">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button onClick={handleDownloadPdf} className="flex items-center px-4 py-2 text-sm font-medium text-white bg-sky-500 hover:bg-sky-600 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500">
          ...
          <>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm">
                ...
                <div className="flex justify-center items-center space-x-6 mt-4 text-gray-600 text-sm">
                    <div className="flex items-center"><MapPin className="w-4 h-4 mr-2 text-gray-400"/> {reportData.nap.address}</div>
                    <div className="flex items-center"><Phone className="w-4 h-4 mr-2 text-gray-400"/> {reportData.nap.phone}</div>
                    <div className="flex items-center"><Building className="w-4 h-4 mr-2 text-gray-400"/> {reportData.businessType}</div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <StatCard icon={<MapPin className="w-6 h-6" />} title="Direction Requests" value={reportData.metrics.directions} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300">
                 <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><TrendingUp className="w-5 h-5 mr-2 text-gray-400"/>Performance Trend</h3>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300">
                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center"><Search className="w-5 h-5 mr-2 text-gray-400"/>Top Trigger Keywords</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 space-y-4">
                    <h3 className="text-lg font-semibold text-gray-800 flex items-center"><Star className="w-5 h-5 mr-2 text-gray-400"/>Reviews Overview</h3>
                    ...
                    <div>
                            ...
                            {reportData.reviews.recent.map(review => (
                                <li key={review.id} className="text-sm bg-gray-50 p-3 rounded-lg">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <p className="font-bold text-gray-800">{review.author} ({review.rating} ★)</p>
                                            <p className="text-gray-600 italic">"{review.text}"</p>
                                        </div>
                                        <span className={`text-xs font-semibold px-2 py-1 rounded-full ${review.responded ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                     ...
                     <div className="pt-4 border-t border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                        <div className="md:col-span-2">
                             ...
                             <p className="text-xs text-gray-500">Share this QR code or shortlink with your customers to make it easy for them to leave a review.</p>
                        ...
                        <div className="flex justify-center md:justify-end">
                            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=https://g.page/r/your-business/review`} alt="Review QR Code" className="w-24 h-24 rounded-md" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg transition-shadow duration-300 border-l-4 border-sky-500">
                <h3 className="text-lg font-semibold text-gray-800 mb-2 flex items-center"><Bot className="w-5 h-5 mr-2 text-sky-500"/>AI-Powered Performance Summary</h3>
